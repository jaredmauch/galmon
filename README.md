...
```
apt-get install protobuf-compiler libcpp-httplib-dev libcurl4-openssl-dev libssl-dev libprotobuf-dev \
...
If this doesn't succeed with an error about `httplib.h`, make sure
`libcpp-httplib-dev` is installed.
...
```
brew install protobuf lzlib zstd cpp-httplib eigen
```
